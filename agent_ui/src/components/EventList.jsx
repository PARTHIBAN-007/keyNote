...
const EventList = ({ events, onSelectEvent }) => {
  return (
    <div className="h-full overflow-y-auto p-6 flex flex-col bg-white">
      <h2 className="text-2xl font-bold text-gray-900 mb-6 sticky top-0 bg-white py-2">Events ({events.length})</h2>
      {events.length === 0 ? (
        <div className="text-gray-500 text-center py-16 flex-1 flex items-center justify-center flex-col">
          <p className="text-lg text-gray-700">No events scheduled yet</p>
          <p className="text-sm text-gray-500 mt-2">Create your first event using the calendar</p>
        </div>
      ) : (
        <ul className="space-y-3 flex-1">
          {events.map((event) => (
            ...
            >
              <div className="font-bold text-gray-900 text-lg group-hover:text-indigo-600 transition">{event.event_name}</div>
